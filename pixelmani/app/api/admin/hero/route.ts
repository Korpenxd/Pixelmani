...
export async function POST(request: Request) {
  ...
  const { data: currentSetting, error: lookupError } =
    await supabaseAdmin
      .from('site_settings')
      .select('value')
  ...
  const currentHeroPath =
    typeof currentSetting?.value === 'string'
      ? currentSetting.value
      : null

  const safeName =
    cleanFileName(fileValue.name) ||
    `hero-${crypto.randomUUID()}.webp`

  const newStoragePath =
    `hero/${crypto.randomUUID()}-${safeName}`
  ...
  if (
    currentHeroPath &&
    currentHeroPath !== newStoragePath
  ) {
