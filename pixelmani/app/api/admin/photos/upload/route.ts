...
export async function POST(request: Request) {
  ...
  try {
    for (const [index, file] of files.entries()) {
      ...
      const originalName =
        originalNames[index]?.trim() || file.name
    ...
    if (insertedPhotoIds.length > 0) {
      ...
      if (cleanupDatabaseError) {
        console.error(
          'Upload database cleanup failed:',
          cleanupDatabaseError.message
        )
      }
    }
    ...
    if (uploadedPaths.length > 0) {
      ...
      if (cleanupStorageError) {
        console.error(
          'Upload Storage cleanup failed:',
          cleanupStorageError.message
        )
      }
    }
